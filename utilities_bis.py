...
def find_barycentre(x, y, method="list_minimize"):
    '''Calculates the index of the barycentre value.
        ...
        '''
    assert(method in ['trapz_minimize', 'list_minimize'])#, 'sum_minimize', 'trapz_list'])
        ...
        def _one_center(x, y):
            ...
            minimized_x = minimize_scalar(find_x, method='Bounded',
                                        bounds=(np.quantile(x, 0.01),
                                                np.quantile(x, 0.99)))
            return minimized_x.x, minimized_y.x

        
    elif method == "list_minimize":
        def _one_center(xx, yy):
            ys = np.sort(np.unique(yy))
            z2 = np.asarray(
                [np.abs(np.trapz(yy[yy<=y_val], x=xx[yy<=y_val]) -\
                        np.trapz(yy[yy>=y_val]-y_val, x=xx[yy>=y_val]))\
                 for y_val in ys])
            yind = np.argmin(z2)
            y_value = (ys[yind] + ys[yind+1])/2
            x_ind = np.argmin(np.abs(np.cumsum(yy) - np.sum(yy)/2))
            x_value = (xx[x_ind]+xx[x_ind+1])/2
            return x_value, y_value
    
    if y.ndim == 1:
        barycentre = _one_center(x, y)
    elif y.ndim == 2:
        barycentre = np.asarray(Parallel(n_jobs=-1)(delayed(_one_center)(y[i])
